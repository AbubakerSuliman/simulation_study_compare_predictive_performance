...
scale_cns = 0.59 #For 50% scale = 0.59, For 70% scale = 0.49, For 90% scale = 0.35
...
while (length(samples_seeds_vec)<40) {
  ...
  ph_formula = "-0.06*((x1/10)^1.25) + 0.23*((x4/100)^2) + 1.297*(x3) - .8*x2 + 1.6*x2*x3 + 0.08*(x5/10) + 0.3*(x5/10)*x2"
  
  sdef <- defSurv(varname = "survTime", formula = ph_formula, scale = scale_t, shape = 0.5)
  
  sdef <- defSurv(sdef, varname = "censorTime", scale = scale_cns, shape = 1)
  
  dtSurv <- genSurv(defData_r(sample_sz = sample_sz_r, seed_r = seed_init), sdef, timeName = "time", censorName = "censorTime",
                    eventName = "event")

    if(between(mean(dtSurv$event==0), cens_rate-0.02, cens_rate+0.02)){
    samples_seeds_vec = append(samples_seeds_vec, seed_init)
    ph_censrate_vec = append(ph_censrate_vec, mean(dtSurv$event==0))
    
    #Add the non-informative variables 
